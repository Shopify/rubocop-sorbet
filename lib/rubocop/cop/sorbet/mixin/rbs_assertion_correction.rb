    ...
    module Sorbet
      ...
      module RBSAssertionCorrection
        ...
        def autocorrect_rbs_assertion(corrector, node, allow_assignment: false, allow_arguments: true)
          ...
          when :receiver
            autocorrect_receiver(corrector, node, expression, annotation)
          when :arguments
        ...
        def autocorrect_receiver(corrector, node, expression, annotation)
          indentation = node.source_range.source_line[/\A[ \t]*/]
          replacement = "#{expression} #: #{annotation}".gsub("\n", "\n#{indentation}")
